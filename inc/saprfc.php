...
class saprfc {
	...
	function printStatus($tags=true) {
		if ($tags) {
			echo $this->getStatusTextLong();
		} else {
			echo strip_tags($this->getStatusTextLong());
		}
	}		
		...
		*/
		...
		if ($result != SAPRFC_OK)
		{
			if ($result == SAPRFC_EXCEPTION ) {
				$this->last_exception = @saprfc_exception($this->func_id);
